...
- format via `pnpm run format` (prettier)
- lint via `pnpm run lint` (prettier --check)
- typecheck via `pnpm run check` (svelte-check)
- database commands: `pnpm db:push`, `pnpm db:generate`, `pnpm db:migrate`, `pnpm db:studio`
...
- `github.com/sveltejs/kit` for SvelteKit framework, routing, load functions, hooks
- `github.com/sveltejs/svelte` for Svelte 5 reactivity, runes, components
- `github.com/mary-ext/atcute` for atcute AT Protocol client libraries
- `github.com/bluesky-social/atproto` for AT Protocol reference implementation
- `github.com/drizzle-team/drizzle-orm` for Drizzle ORM, database schema, queries
